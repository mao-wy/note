# **[#](https://24kcs.github.io/vue3_study/)一.TypeScript快速上手**

## 1. 初识 TypeScript

![TS与JS.png](ts/28ca61cc160c417c8497a00defdca5f0tplv-k3u1fbpfcp-watermark.image)
...
// clean-webpack-plugin 删除webpack打包后的文件夹以及文件
// ts-loader  编译ts文件
...
module.exports = {
  ...
  output: {
    path: resolve('dist'),  //将打包后的目录放在dist目录
    filename: '[name].[contenthash:8].js'   // js文件命名方式 app名称.八位的哈希值.js
  },

  module: {
    rules: [
      {
        test: /\.tsx?$/,    // 通过ts-loader 针对src目录里的 ts 文件 进行编译处理操作
        use: 'ts-loader',
        include: [resolve('src')]
      }
    ]
  ...
  plugins: [
    new CleanWebpackPlugin({    // 将dist目录中以前打包的js文件清理掉
    }),

    new HtmlWebpackPlugin({ // 针对当前，目录pulic中的index.html 进行打包
      template: './public/index.html'
    })
  ],

  resolve: {
    extensions: ['.ts', '.tsx', '.js']  // 针对 ts, tsx, js 文件进行处理 比如引入这类文件可以不写扩展名
  },

  devtool: isProd ? 'cheap-module-source-map' : 'cheap-module-eval-source-map',     // 哪一行代码出现问题或错误的提示信息
